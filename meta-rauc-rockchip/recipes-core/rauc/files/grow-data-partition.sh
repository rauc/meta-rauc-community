...
total_disk_size=$(cat /sys/block/mmcblk1/size)

data_part_size=$(cat /sys/block/mmcblk1/mmcblk1p12/size)
data_part_start=$(cat /sys/block/mmcblk1/mmcblk1p12/start)
...
/usr/bin/umount /dev/mmcblk1p12
/usr/sbin/sgdisk /dev/mmcblk1 -e
/usr/sbin/partprobe
/usr/sbin/parted --script /dev/mmcblk1 resizepart 12 100%
/usr/sbin/resize2fs /dev/mmcblk1p12
/usr/bin/umount /dev/mmcblk1p12
/usr/sbin/e2fsck -fp /dev/mmcblk1p12
/usr/bin/mount /dev/mmcblk1p12 /data/
